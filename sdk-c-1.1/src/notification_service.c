...
int notification_service_create(notification_service_t **svc)
{
    ...
    *svc = (notification_service_t*)regina_malloc(sizeof(notification_service_t));
    ...
    (*svc)->need_sync = 1;
	(*svc)->cmd_max_cache_count = DEFAULT_CMD_CACHE_SIZE;
	(*svc)->cmd_cache_count = 0;
    (void)notification_service_get_and_reset_confirmation_ack(*svc);
    regina_cmdcache_init_cmd_cache_list(&((*svc)->cmd_cache_list));
...
void notification_service_destroy(notification_service_t *svc)
{
    if (svc) {
        ...
        regina_cmdcache_destroy_cmd_cache_list(&(svc->cmd_cache_list));
        regina_free(svc);
    }
}
...
int notification_service_extract_cmdack(notification_service_t *svc,
                                        regina_union_t **ack)
{
    cmd_cache_node_t* tmp = svc->cmd_cache_list.first_node;
    if ((notification_service_has_cmd_cache(svc)) && tmp->cmd_handled) {
        regina_string_t *uuid;

        REGINA_LOG_DEBUG1("[NOTI SVC] Extracting cmd ack: %s.", tmp->cmd_id);

        *ack = regina_union_null_or_string_branch_1_create();
        REGINA_RETURN_IF_NIL(*ack, REGINA_ERR_NOMEM);

        uuid = regina_string_copy_create(tmp->cmd_id);
        REGINA_RETURN_IF_NIL(uuid, REGINA_ERR_NOMEM);

        (*ack)->data = uuid;
        tmp->cmd_handled = 0;
        regina_cmdcache_delete_firstnode_of_list(&(svc->cmd_cache_list));
        svc->cmd_cache_count = svc->cmd_cache_list.cache_count;
                                         ...
                                         regina_union_t *resp)
{
    if ((REGINA_UNION_CMD_SYNC_RESPONSE_OR_NULL_BRANCH_0 == resp->type) &&
        ...
        /* here we handle the cmd data */
        cmd_cache_node_t* tmp = regina_cmdcache_malloc_cmd_cache_node();
        regina_memset(tmp,0,sizeof(cmd_cache_node_t));
        tmp->cmd_body = (char*)regina_malloc(cmd_resp->cmd_body->size);
        REGINA_RETURN_IF_NIL(tmp->cmd_body, REGINA_ERR_NOMEM);
        regina_memcpy(tmp->cmd_body, cmd_resp->cmd_body->buffer,
                      cmd_resp->cmd_body->size);
        tmp->cmd_body_size = cmd_resp->cmd_body->size;
        regina_strcpy(tmp->cmd_id, cmd_resp->uuid->data);
        tmp->cmd_handled = 0;
         /* try to store the cmd data into cmd_cache_list */
        regina_cmdcache_append_node_to_list(&(svc->cmd_cache_list),tmp);
        svc->cmd_cache_count = svc->cmd_cache_list.cache_count;
                                             ...
                                             void *ctx, notification_callback nt_callback)
{
    ...
    if(svc->cmd_cache_count != 0){
       ...
       if(NULL != tmp )
       {
           ...
           nt_callback(ctx, &id, tmp->cmd_body, tmp->cmd_body_size);
           regina_free(tmp->cmd_body);
           tmp->cmd_body = NULL;
           tmp->cmd_handled = 1;
           has_ack = 1;
       }
...
{
    svc->cmd_max_cache_count = size;
    svc->cmd_cache_list.cache_max_count = size;
}
