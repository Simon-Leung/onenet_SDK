                                          ...
                                          const char *priv_key, size_t pvk_len)
{
    ...
    if (0 != err) {
        REGINA_LOG0(FATAL, "Failed to call [mbedtls_ctr_drbg_seed].");
...
static int security_crypto_rsa_codec_flush_encoder(crypto_codec_t *codec, regina_stream_t *output)
{
    ...
    if (0 != err) {
        REGINA_LOG2(ERROR, "Failed to encrypt the data by RSA: block_size=%lu"
                    ", mbedtlsError=%d", rsa_codec->enc_block_size, err);
        return REGINA_ERR_INTERNAL;
    }
