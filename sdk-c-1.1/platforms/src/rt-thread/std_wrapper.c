...
char *regina_strdup(const char *str)
{
    ...
    if(str != NULL) {
        ...
        rt_strncpy(strNew,str, rt_strlen(str));
    }
   return strNew;
}
...
{
    return rt_strncpy(dest, src, rt_strlen(src));
}
...
int regina_snprintf(char *str, size_t size, const char *format, ...)
{
    ...
    ret = rt_vsnprintf(str, size, format, args);
    va_end(args);
    return ret;
}
...
regina_time_t regina_time(regina_time_t *t)
{
     ...
     cur_time = cur_time * 1000 / RT_TICK_PER_SECOND + 1; //to ms
	 return (cur_time / 1000);		//·µ»Øcur_time;
}
