...
typedef struct regina_event_t {
    ...
    int64_t timestamp; /**< timestamp of the sensor data since the Epoch 
                            (00:00:00 UTC, January 1, 1970), measured in milliseconds.
                            if 0 the platform uses the default timestamp. */
} regina_event_t;
...
/**
 * Set the cmd cache size.
 *
 * @param size    the size of the cmd cache size, it is 3 by default.
 * @return        0 means success, otherwise failed
 */

REGINA_API int regina_device_set_cmd_cache_cnt(size_t size);
